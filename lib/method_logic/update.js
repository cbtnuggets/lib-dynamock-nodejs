...
module.exports = ((dynamoInstance, params, table) => {
    ...
    const { UpdateExpression: updateExpression } = params;
    ...
    const supportedExpressionFunctions = {
        if_not_exists: (key, value, updateObj) => {
            ...
            const keyValue = params.ExpressionAttributeNames[key];

            const attributeProperty = params.ExpressionAttributeValues[field] ? field : fallback;
            updateObj[keyValue] = params.ExpressionAttributeValues[attributeProperty];
    ...
    if (_.isNull(objectToModify)) {
        ...
        dynamoInstance.context[table].push(_.merge(updateObj, params.Key));
